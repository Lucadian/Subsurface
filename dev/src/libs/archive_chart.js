...
let data = JSON.parse(fs.readFileSync(path.archive + '/blog/' + years[0] + '.json','utf-8'))[0]

//根据最近更新日期，创建一个往前 130 天的日历
let chart = createCanlendar(years[0],Number(data.moty),Number(data.days[0].dotm),130)
let sheet = [
    60,140,240,360,500
]
traverse:
for(let i=0;i<years.length;i++){
    ...
    let archive = JSON.parse(fs.readFileSync(path.archive + '/blog/' + year + '.json','utf-8'))
...
fs.writeFileSync(path.archive + '/blog/chart.json', JSON.stringify(chart))
